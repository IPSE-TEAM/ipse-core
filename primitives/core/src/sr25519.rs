...
use crate::crypto::{CryptoType, CryptoTypeId, CryptoTypePublicPair, Derive, Public as TraitPublic, UncheckedFrom};
...
impl<'de> Deserialize<'de> for Public {
	...
	where
		D: Deserializer<'de>,
	{
		Public::from_ss58check(&String::deserialize(deserializer)?).map_err(|e| de::Error::custom(format!("{:?}", e)))
	}
}
...
impl<'de> Deserialize<'de> for Signature {
	...
	{
		let signature_hex =
			hex::decode(&String::deserialize(deserializer)?).map_err(|e| de::Error::custom(format!("{:?}", e)))?;
		Ok(Signature::try_from(signature_hex.as_ref()).map_err(|e| de::Error::custom(format!("{:?}", e)))?)
	}
}
...
impl Pair {
	...
	pub fn from_entropy(entropy: &[u8], password: Option<&str>) -> (Pair, Seed) {
		let mini_key: MiniSecretKey =
			mini_secret_from_entropy(entropy, password.unwrap_or("")).expect("32 bytes can always build a key; qed");
...
#[cfg(feature = "std")]
pub fn verify_batch(messages: Vec<&[u8]>, signatures: Vec<&Signature>, pub_keys: Vec<&Public>) -> bool {
...
mod compatibility_test {
	...
	fn verify_known_old_message_should_work() {
		...
		let message =
			b"Verifying that I am the owner of 5G9hQLdsKQswNPgB499DeA5PkFBbgkLPJWkkS6FAM6xGQ8xD. Hash: 221455a3\n";
...
mod test {
	...
	fn sr_test_vector_should_work() {
		...
		assert_eq!(
			public,
			Public::from_raw(hex!("44a996beb1eef7bdcab976ab6d2ca26104834164ecf28fb375600576fcc6eb0f"))
		);
		let message = b"";
	...
	fn seeded_pair_should_work() {
		...
		assert_eq!(
			public,
			Public::from_raw(hex!("741c08a06f41c596608f6774259bd9043304adfa5d3eea62760bd9be97634d63"))
		);
