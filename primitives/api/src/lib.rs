...
///
/// Accepts similar syntax as [`impl_runtime_apis!`](macro.impl_runtime_apis.html) and
/// generates simplified mock implementations of the given runtime apis. The difference in
/// syntax is that the trait does not need to be referenced by a qualified path, methods accept
/// the `&self` parameter and the error type can be specified as associated type. If no error
/// type is specified `String` is used as error type.
///
