...
#[test]
#[should_panic(
	expected = "Could not convert parameter `param` between node and runtime: DecodeFails always fails"
)]
...
fn calling_with_native_else_wasm_and_fail_on_wasm_should_work() {
	let client =
		TestClientBuilder::new().set_execution_strategy(ExecutionStrategy::NativeElseWasm).build();
	let runtime_api = client.runtime_api();
	let block_id = BlockId::Number(client.chain_info().best_number);
...
fn record_proof_works() {
	...
	let mut builder =
		client.new_block_at(&block_id, Default::default(), true).expect("Creates block builder");
