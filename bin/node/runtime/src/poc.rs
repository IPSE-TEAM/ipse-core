...
use crate::constants::{time::{MILLISECS_PER_BLOCK, DAYS, MONTHS}, currency::DOLLARS};
...
impl<T: Trait> Module<T> {
	...
	fn reward_default(reward: BalanceOf<T>) {
		...
		if now >= T::BlockNumber::from(3*MONTHS) {
			...
			// The amount lost by the user is put into the black hole address for the convenience of preliminary testing
