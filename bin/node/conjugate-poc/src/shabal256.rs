...
const A_INIT: [u32; 12] = [
	0x52F84552, 0xE54B7999, 0x2D8EE3EC, 0xB9645191, 0xE0078B86, 0xBB7C44C9, 0xD2B5C1CA, 0xB0D2EB8C, 0x14CE5A45,
	0x22AF50DC, 0xEFFDBC6B, 0xEB21B74A,
];

const B_INIT: [u32; 16] = [
	0xB555C6EE, 0x3E710596, 0xA72A652F, 0x9301515F, 0xDA28C1FA, 0x696FD868, 0x9CB6BF72, 0x0AFE4002, 0xA6E03615,
	0x5138C1D4, 0xBE216306, 0xB38B8890, 0x3EA8B96B, 0x3299ACE4, 0x30924DD4, 0x55CB34A5,
];

const C_INIT: [u32; 16] = [
	0xB405F031, 0xC4233EBA, 0xB3733979, 0xC0DD9D55, 0xC51C28AE, 0xA327B8E1, 0x56C56167, 0xED614433, 0x88B59D60,
	0x60E2CEBA, 0x758B4B8B, 0x83E82A7F, 0xBC968828, 0xE6E00BF7, 0xBA839E55, 0x9B491C60,
];
...
fn perm_elt(
	...
	unsafe {
		*a.get_unchecked_mut(xa0) = (a.get_unchecked(xa0)
			^ ((a.get_unchecked(xa1).wrapping_shl(15u32) | a.get_unchecked(xa1).wrapping_shr(17u32))
				.wrapping_mul(5u32))
			^ xc)
			.wrapping_mul(3u32)
			^ b.get_unchecked(xb1)
			...
			^ xm;
		*b.get_unchecked_mut(xb0) =
			!((b.get_unchecked(xb0).wrapping_shl(1) | b.get_unchecked(xb0).wrapping_shr(31)) ^ a.get_unchecked(xa0));
	}
}
...
mod test {
	...
	const TEST_A_RESULT: [u8; 32] = [
		0xDA, 0x8F, 0x08, 0xC0, 0x2A, 0x67, 0xBA, 0x9A, 0x56, 0xBD, 0xD0, 0x79, 0x8E, 0x48, 0xAE, 0x07, 0x14, 0x21,
		0x5E, 0x09, 0x3B, 0x5B, 0x85, 0x06, 0x49, 0xA3, 0x77, 0x18, 0x99, 0x3F, 0x54, 0xA2,
	];
	const TEST_B_RESULT: [u8; 32] = [
		0xB4, 0x9F, 0x34, 0xBF, 0x51, 0x86, 0x4C, 0x30, 0x53, 0x3C, 0xC4, 0x6C, 0xC2, 0x54, 0x2B, 0xDE, 0xC2, 0xF9,
		0x6F, 0xD0, 0x6F, 0x5C, 0x53, 0x9A, 0xFF, 0x6E, 0xAD, 0x58, 0x83, 0xF7, 0x32, 0x7A,
	];
	const TEST_B_M1: [u32; 16] = [
		0x64636261, 0x68676665, 0x6C6B6A69, 0x706F6E6D, 0x74737271, 0x78777675, 0x302D7A79, 0x34333231, 0x38373635,
		0x42412D39, 0x46454443, 0x4A494847, 0x4E4D4C4B, 0x5251504F, 0x56555453, 0x5A595857,
	];
	const TEST_B_M2: [u32; 16] = [
		0x3231302D, 0x36353433, 0x2D393837, 0x64636261, 0x68676665, 0x6C6B6A69, 0x706F6E6D, 0x74737271, 0x78777675,
		0x00807A79, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
	];
	...
	fn shabal256() {
		...
		// test message B
		let hash_b =
			unsafe { shabal256_hash_fast(&sp_std::mem::transmute::<[u32; 16], [u8; 64]>(TEST_B_M1), &TEST_B_M2) };
