...
### 3.2 命令行界面说明
按住键盘shift键， 点击鼠标右键，选择"在此处打开powershell窗口"，执行.\engraver_cpu.exe
...
```
PS F:\plot> .\engraver_gpu.exe --n 4096 --id 10008312345600028  --path F:\plot\data  --sn 0
```
```
Engraver 2.4.0 - PoC2 Plotter
CPU: Intel(R) Core(TM) i7-10510U CPU @ 1.80GHz [using 8 of 8 cores + AVX2]
thread 'main' panicked at 'get sector size, filename=F:\plot\data', src\utils.rs:247:17
note: Run with `RUST_BACKTRACE=1` for a backtrace.
```
提示指定的P盘文件存放路径不存在，那么我们先新建该文件路径
```
PS F:\plot> mkdir F:\plot\data
```
再次执行P盘操作
```
PS F:\plot> .\engraver_gpu.exe --n 4096 --id 10008312345600028  --path F:\plot\data  --sn 0
...
CPU: Intel(R) Core(TM) i7-10510U CPU @ 1.80GHz [using 8 of 8 cores + AVX2]
RAM: Total=15.79 GiB, Free=7.22 GiB, Usage=1.00 GiB
     HDDcache=1.00 GiB, GPUcache=0.00 GiB,

Numeric ID:  10008312345600028
Start Nonce: 0
Nonces:      4096
Output File: F:\plot\data\10008312345600028_0_4096
...
Generated 4096 nonces in 0h00m32s, 31.96 MiB/s, 7670 nonces/m.
 ```
![avatar]() 
...
```
PS E:\IPSE\plot\win10> .\supervision.exe --mining .\poc-mining.exe
```
...
进入挖矿目录,启动挖矿(按住键盘shift键，点击鼠标右键，选择"在此处打开powershell窗口")，执行如下命令:
```
PS E:\IPSE\plot\win10> .\poc-mining.exe
```
