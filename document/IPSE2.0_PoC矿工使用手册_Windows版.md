...
下载挖矿软件poc-mining最新版本(Windows版本)，压缩包包含三个文件：挖矿配置文件config.yaml、监控工具w10supervision、挖矿软件poc-mining，解压到对应目录:
...
#### 3.3.1 supervision启动挖矿(异常自动重启)
进入挖矿目录,启动挖矿(按住键盘shift键，点击鼠标右键，选择"在此处打开powershell窗口")，执行如下命令:
```
E:\IPSE\plot\win10> w10supervision.exe --mining poc-mining.exe
```
输出日志如下:
```
...
启动挖矿成功，生成以poc-mining.exe.log的日志文件，双击打开即可看到最新日志。

或者安装git，在启动挖矿的目录下(如E:\IPSE\plot\win10>)下，鼠标右键，点击"Git Bash Here"打开git命令行界面，运行tail -f poc-mining.exe.log查看动态日志。

#### 3.3.2 不使用supervision,直接启动挖矿(异常不会自动重启)

进入挖矿目录,启动挖矿(按住键盘shift键，点击鼠标右键，选择"在此处打开powershell窗口")，执行如下命令:
```
E:\IPSE\plot\win10> poc-mining.exe
```
CMD界面就直接可查看日志,输出日志类似以下内容，说明启动挖矿成功，如下:
```
...
```
E:\IPSE\plot\win10>tasklist /fi "imagename eq poc-mining.exe"
```
```
...
要结束进程，则执行以下命令:
```
E:\IPSE\plot\win10> w10supervision.exe --mining poc-mining.exe --stop
```
输出日志如下:
```
