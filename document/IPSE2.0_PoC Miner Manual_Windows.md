...
如果要进行多矿工挖矿，则在当前目录或其他目录新建一个文件夹，拷贝挖矿配置文件config.yaml、监控工具w10supervision、挖矿软件poc-mining至该文件夹下,进入该文件夹。

(1)重名名poc-mining， 比如改成xxxx

(2)根据上面的第3.2章节进行修改配置文件，每个矿工的助记词及P盘id不能重复
...
#### 3.3.1 w10supervision启动挖矿(异常自动重启)
进入挖矿目录,启动挖矿(打开CMD命令行界面)，执行如下命令:
```
...
#### 3.3.2 不使用w10supervision,直接启动挖矿(异常不会自动重启)
