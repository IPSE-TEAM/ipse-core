...
impl Peerset {
	...
	fn on_set_priority_group(&mut self, group_id: &str, peers: HashSet<PeerId>) {
		...
		let (to_insert, to_remove) = {
			...
			let to_remove = current_group.difference(&peers).cloned().collect::<Vec<_>>();
			(to_insert, to_remove)
		};
			...
			for peer_id in self.data.peers().cloned().collect::<Vec<_>>() {
				...
				match self.data.peer(&peer_id) {
					...
					peersstate::Peer::NotConnected(mut peer) => {
						if peer.reputation() == 0 && peer.last_connected_or_discovered() + FORGET_AFTER < now {
							peer.forget_peer();
						} else {
							let before = peer.reputation();
							let after = reput_tick(before);
...
#[cfg(test)]
mod tests {
	use super::{IncomingIndex, Message, Peerset, PeersetConfig, ReputationChange, BANNED_THRESHOLD};
	use futures::prelude::*;
	use libp2p::PeerId;
	...
	fn test_peerset_add_reserved_peer() {
		...
		assert_messages(
			peerset,
			vec![Message::Connect(reserved_peer), Message::Connect(reserved_peer2)],
		);
	}
	...
	fn test_peerset_discovered() {
		...
		assert_messages(peerset, vec![Message::Connect(bootnode), Message::Connect(discovered)]);
	}
