...
use sp_consensus::{
	...
	BlockImport, BlockImportParams, BlockOrigin, Environment, ForkChoiceStrategy, Proposer, SelectChain,
};
...
{
	BasicQueue::new(ManualSealVerifier, block_import, None, None, spawner, registry)
}
...
mod tests {
	...
	async fn instant_seal() {
		...
		let env = ProposerFactory::new(client.clone(), pool.clone(), None);
		// this test checks that blocks are created as soon as transactions are imported into the
		// pool.
		let (sender, receiver) = futures::channel::oneshot::channel();
		let mut sender = Arc::new(Some(sender));
		let commands_stream = pool.pool().validated_pool().import_notification_stream().map(move |_| {
