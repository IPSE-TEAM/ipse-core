...
impl GlobalsSnapshot {
	...
	pub fn apply(&self, instance_wrapper: &InstanceWrapper) -> Result<()> {
		// This is a pointer over saved items, it moves forward when the loop value below takes over
		// it's current value. Since both pointers (`current` and `index` below) are over ordered
		// lists, they eventually hit all equal referenced values.
		let mut current = 0;
