...
impl<B: BlockT> ConsensusGossip<B> {
	...
	fn rebroadcast(&mut self, network: &mut dyn Network<B>) {
		let messages =
			self.messages.iter().map(|entry| (&entry.message_hash, &entry.topic, &entry.message));
		propagate(
			network,
			self.engine_id,
			messages,
...
mod tests {
	...
	fn on_incoming_ignores_discarded_messages() {
		let to_forward = ConsensusGossip::<Block>::new(Arc::new(DiscardAll), [0, 0, 0, 0])
	...
	fn on_incoming_ignores_unregistered_peer() {
		...
		let to_forward = ConsensusGossip::<Block>::new(Arc::new(AllowAll), [0, 0, 0, 0])
			.on_incoming(
				&mut network,
				// Unregistered peer.
				remote.clone(),
