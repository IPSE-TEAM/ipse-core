...
impl<'g, 'p, B: BlockT> ValidatorContext<B> for NetworkContext<'g, 'p, B> {
	...
	fn send_message(&mut self, who: &PeerId, message: Vec<u8>) {
		self.network
			.write_notification(who.clone(), self.gossip.engine_id, message);
	}
...
impl<B: BlockT> ConsensusGossip<B> {
	...
	/// Send all messages with given topic to a peer.
	pub fn send_topic(&mut self, network: &mut dyn Network<B>, who: &PeerId, topic: B::Hash, force: bool) {
	...
	pub fn multicast(&mut self, network: &mut dyn Network<B>, topic: B::Hash, message: Vec<u8>, force: bool) {
		let message_hash = HashFor::<B>::hash(&message);
...
mod tests {
	...
	impl<B: BlockT> Network<B> for NoOpNetwork {
		...
		fn report_peer(&self, peer_id: PeerId, reputation_change: ReputationChange) {
			self.inner
				...
				.unwrap()
				.peer_reports
				.push((peer_id, reputation_change));
		}
	...
	fn on_incoming_ignores_discarded_messages() {
		let to_forward = ConsensusGossip::<Block>::new(Arc::new(DiscardAll), [0, 0, 0, 0]).on_incoming(
	...
	fn on_incoming_ignores_unregistered_peer() {
		...
		let to_forward = ConsensusGossip::<Block>::new(Arc::new(AllowAll), [0, 0, 0, 0]).on_incoming(
			&mut network,
			// Unregistered peer.
			remote.clone(),
			vec![vec![1, 2, 3]],
