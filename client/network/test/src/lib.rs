...
use sc_client_api::{
	...
	BlockBackend, BlockImportNotification, BlockchainEvents, FinalityNotification, FinalityNotifications,
	ImportNotifications,
};
...
use sp_consensus::import_queue::{BasicQueue, BoxFinalityProofImport, BoxJustificationImport, Verifier};
use sp_consensus::Error as ConsensusError;
use sp_consensus::{BlockCheckParams, BlockImportParams, BlockOrigin, ForkChoiceStrategy, JustificationImport};
use sp_core::H256;
...
impl PeersClient {
	...
	pub fn as_block_import<Transaction>(&self) -> BlockImportAdapter<Transaction> {
		match *self {
			PeersClient::Full(ref client, ref _backend) => BlockImportAdapter::new_full(client.clone()),
...
pub trait TestNetFactory: Sized {
	...
	fn make_verifier(&self, client: PeersClient, config: &ProtocolConfig, peer_data: &Self::PeerData)
		-> Self::Verifier;
	...
	fn add_full_peer_with_config(&mut self, config: FullPeerConfig) {
		...
		self.mut_peers(|peers| {
			for peer in peers.iter_mut() {
				peer.network
					.add_known_address(network.service().local_peer_id().clone(), listen_addr.clone());
			}
	...
	fn block_until_connected(&mut self) {
		futures::executor::block_on(futures::future::poll_fn::<(), _>(|cx| self.poll_until_connected(cx)));
	}
...
impl TestNetFactory for TestNet {
	...
	fn make_verifier(&self, _client: PeersClient, _config: &ProtocolConfig, _peer_data: &()) -> Self::Verifier {
