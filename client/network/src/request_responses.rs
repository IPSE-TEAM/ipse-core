...
use libp2p::{
	...
	request_response::{
		ProtocolSupport, RequestResponse, RequestResponseCodec, RequestResponseConfig, RequestResponseEvent,
		RequestResponseMessage, ResponseChannel,
	},
	swarm::{
		protocols_handler::multi::MultiHandler, NetworkBehaviour, NetworkBehaviourAction, PollParameters,
		ProtocolsHandler,
	},
};
use std::{
...
pub struct RequestResponsesBehaviour {
	...
	protocols: HashMap<Cow<'static, str>, (RequestResponse<GenericCodec>, Option<mpsc::Sender<IncomingRequest>>)>,
	...
	pending_responses: stream::FuturesUnordered<Pin<Box<dyn Future<Output = RequestProcessingOutcome> + Send>>>,
}
...
impl NetworkBehaviour for RequestResponsesBehaviour {
	type ProtocolsHandler = MultiHandler<String, <RequestResponse<GenericCodec> as NetworkBehaviour>::ProtocolsHandler>;
	type OutEvent = Event;
...
impl RequestResponseCodec for GenericCodec {
	...
	async fn read_request<T>(&mut self, _: &Self::Protocol, mut io: &mut T) -> io::Result<Self::Request>
	where
	...
	async fn read_response<T>(&mut self, _: &Self::Protocol, mut io: &mut T) -> io::Result<Self::Response>
	where
		T: AsyncRead + Unpin + Send,
	{
		...
		let length = match unsigned_varint::aio::read_usize(&mut io).await {
			Ok(l) => l,
			Err(unsigned_varint::io::ReadError::Io(err)) if matches!(err.kind(), io::ErrorKind::UnexpectedEof) => {
				return Ok(Err(()))
			}
