...
use crate::{
	gas::Gas, BalanceOf, Config, ContractAddressFor, ContractInfo, ContractInfoOf, Error, GenesisConfig, Module,
	RawAliveContractInfo, RawEvent, RuntimeReturnCode, Schedule, Trait, TrieId, TrieIdGenerator,
};
...
fn instantiate_and_call_and_deposit_event() {
	...
	ExtBuilder::default().existential_deposit(100).build().execute_with(|| {
		...
		let creation = Contracts::instantiate(Origin::signed(ALICE), subsistence, GAS_LIMIT, code_hash.into(), vec![]);
		...
		pretty_assertions::assert_eq!(
			...
			vec![
				...
				EventRecord {
					phase: Phase::Initialization,
					event: MetaEvent::balances(pallet_balances::RawEvent::Endowed(BOB, subsistence)),
					topics: vec![],
				},
				EventRecord {
					phase: Phase::Initialization,
					event: MetaEvent::balances(pallet_balances::RawEvent::Transfer(ALICE, BOB, subsistence)),
					topics: vec![],
				},
				EventRecord {
					phase: Phase::Initialization,
...
fn removals(trigger_call: impl Fn() -> bool) {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		// Create
		let _ = Balances::deposit_creating(&ALICE, 1_000_000);
		let subsistence_threshold = Balances::minimum_balance() + <Test as Trait>::TombstoneDeposit::get();
		assert_ok!(Contracts::put_code(Origin::signed(ALICE), wasm.clone()));
		assert_ok!(Contracts::instantiate(
			Origin::signed(ALICE),
...
fn restoration(test_different_storage: bool, test_restore_to_with_dirty_storage: bool) {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		if test_different_storage || test_restore_to_with_dirty_storage {
			...
			assert_err_ignore_postinfo!(perform_the_restoration(), Error::<Test>::ContractTrapped,);

			assert!(ContractInfoOf::<Test>::get(BOB).unwrap().get_tombstone().is_some());
			let django_contract = ContractInfoOf::<Test>::get(DJANGO).unwrap().get_alive().unwrap();
			...
			assert_eq!(
				...
				vec![
					...
					EventRecord {
						phase: Phase::Initialization,
						event: MetaEvent::contracts(RawEvent::Restored(DJANGO, BOB, bob_contract.code_hash, 50)),
						topics: vec![],
					},
				]
			);
...
fn cannot_self_destruct_through_draning() {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		assert_matches!(ContractInfoOf::<Test>::get(BOB), Some(ContractInfo::Alive(_)));

		// Call BOB which makes it send all funds to the zero address
		// The contract code asserts that the correct error value is returned.
		assert_ok!(Contracts::call(Origin::signed(ALICE), BOB, 0, GAS_LIMIT, vec![],));
	});
}
...
fn destroy_contract_and_transfer_funds() {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		assert_matches!(ContractInfoOf::<Test>::get(CHARLIE), Some(ContractInfo::Alive(_)));
...
fn cannot_self_destruct_in_constructor() {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		assert_err_ignore_postinfo!(
			Contracts::instantiate(Origin::signed(ALICE), 100_000, GAS_LIMIT, code_hash.into(), vec![],),
...
fn crypto_hashes() {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		for (n, (hash_fn, expected_size)) in test_cases.iter().enumerate() {
			...
			let result = <Module<Test>>::bare_call(ALICE, BOB, 0, GAS_LIMIT, params).0.unwrap();
...
fn transfer_return_code() {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		let result = Contracts::bare_call(ALICE, BOB, 0, GAS_LIMIT, vec![]).0.unwrap();
...
fn call_return_code() {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		let result = Contracts::bare_call(ALICE, BOB, 0, GAS_LIMIT, vec![0]).0.unwrap();
		...
		let result = Contracts::bare_call(ALICE, BOB, 0, GAS_LIMIT, vec![1]).0.unwrap();
		assert_return_code!(result, RuntimeReturnCode::CalleeReverted);

		// Contract has enough balance but callee traps because "2" is passed.
		let result = Contracts::bare_call(ALICE, BOB, 0, GAS_LIMIT, vec![2]).0.unwrap();
...
fn instantiate_return_code() {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		// Contract has only the minimal balance so any transfer will return BelowSubsistence.
		let result = Contracts::bare_call(ALICE, BOB, 0, GAS_LIMIT, vec![0; 33]).0.unwrap();
		assert_return_code!(result, RuntimeReturnCode::BelowSubsistenceThreshold);
