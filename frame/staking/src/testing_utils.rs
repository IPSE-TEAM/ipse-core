...
pub fn create_validators_with_nominators_for_era<T: Trait>(
	...
	let mut validators_stash: Vec<<T::Lookup as StaticLookup>::Source> = Vec::with_capacity(validators as usize);
	...
	for j in 0..nominators {
		...
		let (_n_stash, n_controller) =
			create_stash_controller::<T>(u32::max_value() - j, balance_factor, RewardDestination::Staked)?;

		// Have them randomly validate
		let mut available_validators = validator_choosen.clone();
		let mut selected_validators: Vec<<T::Lookup as StaticLookup>::Source> = Vec::with_capacity(edge_per_nominator);
...
pub fn get_weak_solution<T: Trait>(
	...
	<Validators<T>>::iter().for_each(|(who, _p)| {
		*backing_stake_of.entry(who.clone()).or_insert_with(|| Zero::zero()) += <Module<T>>::slashable_balance_of(&who)
	});
	...
	winners.iter().for_each(|w| {
		staked_assignments.push(StakedAssignment {
			...
			distribution: vec![(
				w.clone(),
				<T::CurrencyToVote as Convert<BalanceOf<T>, u64>>::convert(<Module<T>>::slashable_balance_of(&w))
					as ExtendedBalance,
			)],
		})
	});
	...
	let score = {
		let staked = assignment_ratio_to_staked::<_, OffchainAccuracy, _>(low_accuracy_assignment.clone(), stake_of);

		let support_map = build_support_map::<T::AccountId>(winners.as_slice(), staked.as_slice()).unwrap();
		evaluate_support::<T::AccountId>(&support_map)
	};

	// compact encode the assignment.
	let compact =
		CompactAssignments::from_assignment(low_accuracy_assignment, nominator_index, validator_index).unwrap();
...
pub fn get_seq_phragmen_solution<T: Trait>(
	...
	offchain_election::prepare_submission::<T>(assignments, winners, do_reduce, T::MaximumBlockWeight::get()).unwrap()
}
